...
datafilename = "G:\\Courses\\Spring 18\\DBMS\\dbms-repo\\DBMS\\data\\data_showmethecurry.csv"
...
def main():
    ...
    for aElem in aArr:
        try:
            ...
            response = urllib2.urlopen(req)
            soup = BeautifulSoup(response.read(), "html.parser")
            aRecipeArr=soup.find_all(class_="konafilter")[0].find_all("a")
            for aRecipe in aRecipeArr:
                link=aRecipe['href']
                if link not in recipeUrlSet:
                    recipe_set.add(link)
        except Exception as e:
...
def parseRecipePage(url):
    ...
    try:
        ...
        name=soup.select("span[class='current']")[0].get_text().strip()
        ...
        try:
            desc=soup.select("div[class='post-content'] p")[0].get_text().replace("\"\"", "\"").strip()
            log_debug("desc = " + desc)
            row.append(desc)
        except Exception as e:
